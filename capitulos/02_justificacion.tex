...
una relevancia muy alta en el campo de la informatica sobretodo en la seguridad. Pero como he
...
A simple vista podemos ver que el código no optimizado es mas largo que el optimizado. Si nos
fijamos mas en detalle en el codigo original vemos que tenemos un bucle \mintinline{c}{while(1)}
...
del bucle se esta comprobando todo el rato con la instrucción \mintinline{asm}{test eax, eax}
la cual cosa si pensamos es inecesaria ya que la condición del bucle siempre es cierta.
...
END
    \end{minted}
    \caption[]{(\cite{EuclidianCode})}
    \label{cod:EuclidianCodeAsmNoOpt}
\end{mycode}

Lo que a simple vista vemos que se puede optimizar, el compilador lo ve y vemos que en el codigo 
...
a optimizado eliminando codigo inecesario y que era costoso computacionalmente. \textbf{¿Pero que
repercusion tiene esto a la hora de aplicar ingeniería inversa?} Si tan solo disponemos del código 
optimizado lo que llegaremos a inferir es que la condición del bucle no es \mintinline{c}{while(1)}
si no que es \mintinline{c}{while(r != 0)} y que ademas el la estrucutra del \mintinline{c}{if} desaparece.
En la figura \ref{cod:EuclidianCodeIng} podemos ver el resultado de aplicar ingieneria inversa sobre
